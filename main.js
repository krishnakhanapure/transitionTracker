...
app.post('/tt/saml/consume', authenticate.authenticateUser, (req, res) => {
  passportStrategy.passport.authenticate('saml', (err, user, info) => {
    if (err) {
      console.log(err);
      return res.sendStatus(500);
    }
    req.login(user, () => {
      res.redirect(req.session.currentPath ? req.session.currentPath : '/tt/mainDashboard');
    })
  })(req, res)
});
